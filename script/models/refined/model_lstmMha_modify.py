...
class AttentionGRU(nn.Module):
    def __init__(self, args):
        ...
        self.smi_fc_layers = nn.ModuleList([nn.Linear(self.hidden_dim_lstm * 2, self.output_dim) for _ in range(self.num_attention_heads)])
        ...
        self.ep_fc = nn.Linear(self.hidden_dim_lstm * 2, self.output_dim)
        self.fc1 = nn.Linear(2*(self.output_dim + self.num_features_x), self.n_output)
    ...
    def forward(self, encodedSmi, ecfp, x, edge_index, batch):
        ...
        combined_features = torch.cat((smi_attended_out, ep_attended_out, gc), dim=1)
