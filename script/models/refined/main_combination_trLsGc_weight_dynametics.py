...
weights = torch.tensor([0.15, 0.80, 0.05], requires_grad=True, dtype=torch.float)
...
savePath = '../../../result/seed42/refined/dyweight/{}_rmseMae_{}_{}_{}_weightGradient_com.csv'.format(label, batch_size, epochs, random_state)
rmseAndMae(yResult, savePath)
savePath = '../../../result/seed42/refined/dyweight/{}_weight_{}_{}_{}_weight_com.csv'.format(label, batch_size, epochs, random_state)
df_weight = pd.DataFrame(weightDic.items(), columns=['Key', 'Value'])
df_weight.to_csv(savePath, index=False)


savePath = '../../../result/seed42/refined/dyweight/{}_scatter_predited_{}_{}_{}_com.png'.format(label, batch_size, epochs, random_state)
...
result['y_pred'] = yPred
result['y'] = ySour
resultPath = '../../../result/seed42/refined/dyweight/{}-result-{}-{}-{}-com.csv'.format(label, batch_size, epochs, random_state)
resultDf = pd.DataFrame(result)
resultDf.to_csv(resultPath, index=False)
print(resultPath + '\tsaved successfully')
print('\n')

savePath = '../../../result/seed42/refined/dyweight/{}_loss_{}_{}_{}_weigh_com.png'.format(label, batch_size, epochs, random_state)
plotLoss(resultLoss['losses_train'], resultLoss['losses_val'], savePath)

savePath = '../../../result/seed42/refined/dyweight/{}_validation_{}_{}_{}_data.csv'.format(label, batch_size, epochs, random_state)
