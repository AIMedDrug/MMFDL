...
class comModel(nn.Module):
    def __init__(self, args):
        ...
        self.emb = Embeddings(self.num_features_smi, self.hidden_dim, self.num_features_smi, self.dropout)
        ...
        self.smi_fc2 = nn.Linear(self.hidden_dim, self.output_dim)
        self.ep_fc1 = nn.Linear(self.hidden_dim*2, self.output_dim)
        self.out_fc = nn.Linear(2*(self.output_dim + self.num_features_x), self.n_output)
    ...
    def forward(self, encodedSmi, encodedSmi_mask, ecfp, x, edge_index, batch):
        ...
        print(smi_emb.shape)
        print(smi_encoded.shape)
        exit()
        smi_ln = self.smi_fc1(smi_encoded)
        smi_ln = smi_ln.view(smi_ln.shape[0], -1)
        smi_out = self.smi_fc2(smi_ln)
       
        ep_gru, _ = self.ep_gru(ecfp)
        ep_norm = self.ep_norm1(ep_gru)
        ep_attended_out = 0
        for i in range(self.num_attention_heads):
            ep_attention_weights = torch.softmax(self.ep_attention_layers[i](ep_norm), dim=1)
            ep_linear = self.ep_fc_layers[i](ep_norm)
            ep_attended_out += ep_attention_weights * ep_linear
        ep_attended_out /= self.num_attention_heads
        ep_ln = self.ep_fc1(ep_norm)
        ep_out = self.ep_norm2(ep_attended_out + ep_ln)
        ...
        combined_features = torch.cat((smi_out, ep_out, gc), dim=1)
        out = self.out_fc(combined_features)
        out = out.view(1, -1)
        return out
